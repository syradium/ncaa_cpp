...
double FindRoot(double a, double b, const double &a0, const double &b0, const vector<double> &p0,
...
int FR(const double &a, const double &b, matrix<double> &t, vector<double> &p0, vector<double> &q0, const int &ii0) {
    ...
    z[0] = FindRoot(leftBound, q0[0] - ::eps * 10, a0, b0, p0, q0, j);

    for (int i = 0; i < j; ++i)
        z[i + 1] = FindRoot(q0[i] + ::eps * 10, q0[i + 1] - ::eps * 10, a0, b0, p0, q0, j);
    z[j + 1] = FindRoot(q0[j] + ::eps * 10, rightBound, a0, b0, p0, q0, j);
...
bool SConsist(int i, const dvector &tAngle, const dvector &pAngle, dvector &M, dvector &N, const dvector &E0,
    ...
    do {
        ...
        auto green_matrix_element = [&H](int index1, int index2, complex<double> vector1,
                                         complex<double> vector2) -> matrix<double> {
        ...
        n_new = Nu + Nd;
        m_new = (Nu - Nd) * cos(tAngle[i]) -
                ((SFp - SFn) * cos(pAngle[i]) - (AFp - AFn) * sin(pAngle[i])) * sin(tAngle[i]);
                    ...
                    do {
                        ...
                        while (j < L) {
                            ...
                            for (int i = 0; i < CORE_COUNT && j < L; ++i, ++j) {
                                threads.add_thread(new boost::thread(&SConsistThreaded, j, boost::ref(resultNM),
                                                                     std::make_pair(thetaAngles, phiAngles),
                                                                     M, N, std::make_pair(E0, U0),
                                                                     hopingIntegrals, boost::ref(Energy),
                                                                     boost::ref(isConsist)));
                            }
                            threads.join_all();
//							if( interrupt == true )
...
void buildSelfCSolution(dvector &thetaAngles, const dvector &phiAngles, dvector &magneticMoments, dvector &electronsNumber, const dvector &E0, const dvector &U0, const dmatrix &hopingIntegrals, dvector &Energy) {
    ...
    do {
        isConsist = true;
        for (int i = 0; i < L; ++i)
            isConsist &= SConsist(i, thetaAngles, phiAngles, magneticMoments, electronsNumber, E0, U0, hopingIntegrals,
                                  Energy);
        if (++iterations == 50)
            throw "#Infinite consist cluster loop.";
    }
...
void EnergySurfaceTask(const dmatrix &results) {
    auto result_energy_path = "temp.txt";
    std::ofstream logFile(result_energy_path, std::ios_base::out | std::ios_base::trunc);
    for (int i = 0; i < results.size(); ++i) {
        for (int j = 0; j < results[i].size(); ++j)
